 ...
 * 	Caller should ensure to hold rcu_lock()
 * 	This function is a rcu variant of br_vlan_get_info()
 * 	net_bridge_vlan_group is rcu protected in net_bridge_port,
 * 	so rcu lock will prevent any mishap.
 */
int br_vlan_get_info_rcu(const struct net_device *dev, u16 vid, struct bridge_vlan_info *p_vinfo)
{
	struct net_bridge_vlan_group *vg;
	struct net_bridge_vlan *v;
	struct net_bridge_port *p;
